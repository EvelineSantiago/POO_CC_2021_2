...
int main() {
    ...
    sf::Texture fireboy_tex { loadTexture("fireboy.png") };
    sf::Texture watergirl_tex { loadTexture("watergirl.png") };
    sf::Texture background_tex { loadTexture("fundo.jpg") };

    const int STEP {100};

    Entity fireboy(1, 1, STEP, fireboy_tex);
    Entity watergirl(3, 3, STEP, watergirl_tex);
    Board board(7, 5, STEP, background_tex);

    sf::RenderWindow window(sf::VideoMode(board.nc * STEP, board.nl * STEP), "Fireboy & Watergirl!");
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed) {
                ...
                moveEntity(event.key.code, fireboy, {sf::Keyboard::Left, sf::Keyboard::Up, sf::Keyboard::Right, sf::Keyboard::Down});
                moveEntity(event.key.code, watergirl, {sf::Keyboard::A, sf::Keyboard::W, sf::Keyboard::D, sf::Keyboard::S});
            }
        }
